...
export function NotificationsMenu() {
    const { alerts, isLoading, refresh } = useNotifications()
    ...
    const title = (
        <span className="notif-toggle">
            ...
            {count > 0 && <Badge bg="danger" pill className="notif-badge">{count > 9 ? '9+' : count}</Badge>}
        </span>
    )

    const openAlert = (meterId: string) => {
        ...
        <NavDropdown title={title} align="end" className="notif-menu" onClick={() => refresh()} aria-label="Powiadomienia">
            ...
            {alerts.length === 0 ? (
                ...
                alerts.slice(0, 8).map((alert, index) => (
                    <NavDropdown.Item key={`${alert.meterId}-${index}`} onClick={() => openAlert(alert.meterId)} className="notif-item">
                        <div className="d-flex align-items-start">
                            ...
                            />
                            <div>
                                <div className="fw-semibold small">{alert.meterName}</div>
